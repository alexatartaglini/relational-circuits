...
def same_diff_eval(config, model, dataloader):
    vanilla_correct = []
    ablate_correct = []

    for idx, batch in enumerate(dataloader):
        batch = {k: v.to(config["device"]) for k, v in batch[0].items()}
        ### Get full model accuracy
        model.use_masks(False)
        outputs = model(**batch).logits
        preds = outputs.argmax(1)
        vanilla_correct += list((preds == batch["labels"]).cpu())

        ## Get ablated model accuracy
        model.use_masks(True)
        outputs = model(**batch).logits
        preds = outputs.argmax(1)
        ablate_correct += list((preds == batch["labels"]).cpu())

    model.use_masks(True)

    return {
        "vanilla_acc": np.sum(vanilla_correct) / len(vanilla_correct),
        "ablated_acc": np.sum(ablate_correct) / len(ablate_correct),
    }
