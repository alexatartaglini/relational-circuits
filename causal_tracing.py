...
def get_score(patch_logits, same_log_diff, corr_log_diff):
    patched_logits_diff = patch_logits[0, 1] - patch_logits[0, 0]
    return (patched_logits_diff - corr_log_diff)/(same_log_diff - corr_log_diff)
...
with torch.no_grad():
    ...
    corr_log_diff = corr_logits[0, 1] - corr_logits[0, 0]

    hooks = add_cache_hooks(model)
    model(same) # patching from clean into corrupted
    remove_hooks(hooks)
    ...
    for patch_type in ["attn", "mlp", "stream"]:
        ...
        for layer in range(12):
            for stream_index in range(num_streams):
                ...
                patch_logits = model(corrupt).logits # Patched clean into corrupted
                score = get_score(patch_logits, same_log_diff, corr_log_diff)
