...
class ViTPatchSelfAttention(nn.Module):
    def __init__(self, config: ViTConfig) -> None:
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(
            config, "embedding_size"
    ...
    def forward(
        ...
        if attention_map_strength > 0:
            ...
            attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))
            if attention_map_strength >= 1.0:
                attention_scores = attention_scores + (attention_maps * attention_map_strength)
...
class ViTPatchAttention(nn.Module):
    def __init__(self, config: ViTConfig) -> None:
        super().__init__()
        self.attention = ViTPatchSelfAttention(config)
        self.output = ViTSelfOutput(config)
        self.pruned_heads = set()
...
class ViTLayer(nn.Module):
    ...
    def __init__(self, config: ViTConfig, patch=False) -> None:
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        if patch:
            self.attention = VIT_ATTENTION_CLASSES["patch"](config)
        else:
...
class ViTEncoder(nn.Module):
    def __init__(self, config: ViTConfig) -> None:
        ...
        self.layer = nn.ModuleList(
            [
                ViTLayer(config, patch=(l in config.patch_layer_list))
...
class AttnMapViTModel(ViTPreTrainedModel):
    def __init__(
        self,
        config: ViTConfig,
...
class AttnMapViTForImageClassification(ViTPreTrainedModel):
    def __init__(self, config: ViTConfig) -> None:
        super().__init__(config)

        self.num_labels = config.num_labels
        self.vit = AttnMapViTModel(config, add_pooling_layer=False)
