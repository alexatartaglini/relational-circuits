...
if __name__ == "__main__":
    ...
    # Create blank image to inject images
    blank = Image.new("RGB", (224, 224), (255, 255, 255))

    image_processor, tl_model = load_tl_model(
        model_path,
        pretrain,
        patch_size,
    )
    ...
    if pretrain == "clip":
        blank = image_processor(
            images=np.array(blank, dtype=np.float32), return_tensors="pt"
        )["pixel_values"][0].to("cuda")
    else:
        blank = image_processor.preprocess(
            np.array(blank, dtype=np.float32),
            return_tensors="pt",
        )["pixel_values"][0].to("cuda")

    blank = blank.unsqueeze(0).repeat((64, 1, 1, 1))
    ...
    for layer in range(0, 12):
        ...
        stds = torch.std(2 * layer_embeds, dim=0)
        ...
        def interpolate(embs):
            ...
            return (embs[choices[0]] * 1.0) + (embs[choices[1]] * 1.0)
